...
<script type="text/javascript" src="../js/script.js"></script> 
 ...
 $(document).ready(function() {
   ...
   var sheet=document.getElementById("src");
   var val =  sheet.options[sheet.selectedIndex].text;
   var selected =document.getElementById("selectedsrc");
   selected.value=val;
});
...
where edges.source_id=A.CITY_ID AND edges.dest_id=B.CITY_ID AND A.ROW_STATUS=1 AND B.ROW_STATUS=1";
$stmt = oci_parse($conn,$query);
oci_execute($stmt);
while(oci_fetch($stmt)){
...
<tr>
<td> Source City</td>
<td><select name=src id=src><?php populateDropDown() ?></select></td>
</tr>
...
<tr>
	...
	<td><input type=datetime-local name=deptime min="<?php echo date("Y-m-d");?>" required></td>
</tr>

<tr>
	<td>BUS</td>
	<td><select name=bus><?php 
	include 'connection.php';
//echo "Hello";
...
oci_execute($stmt);
while(oci_fetch($stmt)){
 $id=oci_result($stmt,'BUS_ID');
 $model=oci_result($stmt,'MODEL');
 $capacity=oci_result($stmt,'CAPACITY');
 $type=oci_result($stmt,'TYPE');
 
 echo "<option id=".$id." value=".$id.">".$model."(".$capacity.") ".$type."</option>";
}
	?></select></td>
</tr>
...
<input type=hidden value="" id=selectedsrc >
